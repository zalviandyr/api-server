...
const {
    toCamelCase, errorResponse, toMp3,
        ...
        try {
            ...
            (async () => {
                ...
                try {
                    ...
                    resolve({
                        title, size, duration, ext: 'mp3', url: urlResult,
                    })
                } catch (err) {
                    rejects(errorResponse(500, err.message))
                    await browser.close()
                ...
                (async () => {
                    ...
                    try {
                        ...
                        const result = await page.evaluate((element) => {
                            const firstSearch = element.querySelectorAll('ytd-video-renderer')[0]

                            const thumb = firstSearch.querySelector('div > ytd-thumbnail > a > yt-img-shadow > img').getAttribute('src')
                            const duration = firstSearch.querySelector('div > ytd-thumbnail > a > div > ytd-thumbnail-overlay-time-status-renderer').innerText
                            const title = firstSearch.querySelector('div > div > div > div > h3').innerText
                            const link = (() => {
                                const temp = firstSearch.querySelector('div > div > div > div > h3 > a').getAttribute('href')
                                return 'https://www.youtube.com' + temp
                            })()
                            const views = firstSearch.querySelector('div > div > div#meta > ytd-video-meta-block > div#metadata > div#metadata-line > span:nth-child(1)').textContent
                            const upload = firstSearch.querySelector('div > div > div#meta > ytd-video-meta-block > div#metadata > div#metadata-line > span:nth-child(2)').textContent
                            const channelLink = (() => {
                                const temp = firstSearch.querySelector('div > div > div#channel-info > a').getAttribute('href')
                                return 'https://www.youtube.com' + temp
                            })()
                            const channelTitle = firstSearch.querySelector('div > div > div#channel-info > ytd-channel-name').innerText
                            const description = firstSearch.querySelector('div > div > yt-formatted-string#description-text').innerText
