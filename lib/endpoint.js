    ...
    kusonime: (query) => new Promise((resolve, rejects) => {
        ...
        (async () => {
            ...
            try {
                ...
                // const xpathSearchResult = '//*[@id="venkonten"]/div[7]/div[1]/div/div/div[1]/div[2]/div/ul'
    ...
    drakorasia: (query) => new Promise((resolve, rejects) => {
        ...
        (async () => {
            ...
            try {
                ...
                const validUrl = urlParse(firstResultUrl)
                if (validUrl.hostname && validUrl.protocol) {
                    ...
                    const containerWrapperResult = await page.evaluate((element) => {
                            ...
                            for (let i = 0; i < tempEpisodes.length; i++) {
                                ...
                                for (let j = 0; j < resolutionDownload.length; j++) {
                                    ...
                                    for (let k = 0; k < downloadList.length; k++) {
                                        const title = (() => {
                                            for (let l = 0; l < legendDownloadTitle.length; l++) {
                                                if (downloadList[k].textContent === legendDownloadTitle[l].shortName) {
                                                    return legendDownloadTitle[l].name
                                                }
                                            }
                                            return downloadList[k].textContent
                                        })()
                    ...
                    // jika tidak result
                    rejects(errorResponse(400, firstResultUrl))
                }
            } catch (err) {
                rejects(errorResponse(500, err.message))
                await browser.close()
