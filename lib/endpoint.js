const ytdl = require('ytdl-core')

// my library
const { formatBytes, getVideoID } = require('./helpers')

const endpoint = {
    ytVideo: (encrypted, hostname, query) => new Promise((resolve) => {
        const url = query.url
        const ytVideo = ytdl(url, { filter: (format) => format.container === 'mp4' })
        ytVideo.on('info', (info, format) => {
            ytVideo.on('progress', (chunkSize, downloaded, size) => {
                // check video id
                const videoID = getVideoID(url)
                const videoTitle = info.videoDetails.title.replace(/[^a-zA-Z0-9&()]/g, ' ').trim()
                const protocol = (encrypted == null) ? 'http://' : 'https://'

                const result = {
                    title: videoTitle,
                    ext: format.container,
                    size: formatBytes(size),
                    url: protocol + hostname + `/api/yt-video/download?id=${videoID}&title=${videoTitle}&ext=${format.container}`,
                }

                resolve(result)
            })
        })
    }),

    ytAudio: (encrypted, hostname, query) => new Promise((resolve) => {
        const url = query.url
        const ytAudio = ytdl(url, { quality: 'highestaudio', filter: 'audioonly' })
        ytAudio.on('info', (info, format) => {
            ytAudio.on('progress', (chunkSize, downloaded, size) => {
                // check video id
                const videoID = getVideoID(url)
                const videoTitle = info.videoDetails.title.replace(/[^a-zA-Z0-9&()]/g, ' ').trim()
                const protocol = (encrypted == null) ? 'http://' : 'https://'

                const result = {
                    title: videoTitle,
                    ext: format.container,
                    size: formatBytes(size),
                    url: protocol + hostname + `/api/yt-audio/download?id=${videoID}&title=${videoTitle}&ext=${format.container}`,
                }

                resolve(result)
            })
        })
    }),
}

module.exports = endpoint
