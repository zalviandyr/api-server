...
const endpoint = {
     ...
     * query => search
     */
    movie: (query) => new Promise((resolve, rejects) => {
        if (!query.search) {
            rejects(errorResponse(400, 'Silahkan isi query search, contoh: ?search=spiderman'))
            return
        }
        const keyword = query.search.replace(/ /g, '+')
            ...
            (async () => {
                ...
                try {
                    ...
                    const urlFirstResult = await page.evaluate((element) => {
                        ...
                        return listSearch[0].querySelector('div > div > div > a').getAttribute('href')
                    }, elementsSearchResult)
                    ...
                    if (urlFirstResult) {
                        ...
                        const detailResult = await page.evaluate((element) => {
                            ...
                            const thumb = content.querySelector('div:nth-child(1) > img').getAttribute('src')
                            ...
                            const synopsis = content.querySelector('div:nth-child(3) > div > p').innerText
                        ...
                        const downloadResult = await downloadPage.evaluate((element) => {
                            ...
                            for (let i = 0; i < listTr.length; i++) {
                                ...
                                temp.push({ provider, resolution, downloadLink })
                            }
                        ...
                        resolve(Object.assign(detailResult, downloadResult))
                    } else {
                        rejects(errorResponse(404, 'Maaf, tidak hasil untuk mu'))
                    }
                } catch (err) {
                    rejects(errorResponse(500, err.message))
                    await browser.close()
