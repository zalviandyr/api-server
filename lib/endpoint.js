...
const { urlBmkg } = require('./helpers/values')
...
const endpoint = {
    ...
    cuaca: (query) => new Promise((resolve) => {
        ...
        if (query.kabupaten) {
            ...
            fs.readFile(path, 'utf8', (err, data) => {
                ...
                if (!found) {
                    result.message = 'Kabupaten atau kota tidak ada'
                    resolve(result)
                }
            })
        } else {
            result.message = 'Silahkan masukkan query kabupaten, contoh ?kabupaten=jambi'
            resolve(result)
        }
    }),
