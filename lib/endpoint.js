...
const endpoint = {
    ...
    kusonime: (query) => new Promise((resolve, rejects) => {
        ...
        (async () => {
            const browser = await puppeteer.launch({ args: ["--proxy-server='direct://'", '--proxy-bypass-list=*', '--no-sandbox', '--disable-setuid-sandbox'], headless: true })
            const page = await browser.newPage()
            // user agent
            await page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.100 Safari/537.36')

            // Leap of faith
            const label = 'Leap of faith - Kusonime'
            console.time(label)
            ...
            // TODO delete this if dont need anymore
                ...
                const download = (() => {
                    ...
                    for (let i = 0; i < downloadUrl.length; i++) {
                        ...
                        for (let j = 0; j < aPerResolution.length; j++) {
                            const shortLink = aPerResolution[j].getAttribute('href')
            ...
            for (let i = 0; i < result.download.length; i++) {
                ...
                for (let j = 0; j < downloadList.length; j++) {
                    const queryData = urlParse(downloadList[j].shortLink, { parseQueryString: true }).query
                    downloadList[j].shortLink = queryData.url
                }
            }
