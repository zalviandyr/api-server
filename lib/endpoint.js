...
const endpoint = {
    ...
    penyakit: (query) => new Promise((resolve, rejects) => {
        ...
        (async () => {
            ...
            const result = await page.evaluate((element) => {
                ...
                const indexToSplice2 = getArrayFromIndex.findIndex((f) => f.includes('Anda tidak memiliki'))
    ...
    surat: (query) => new Promise((resolve, rejects) => {
        if (query.surat) {
            ...
            rejects(errorResponse(400, 'Silahkan isi query surat, contoh: ?surat=1'))
        }
        ...
        if (query.surat && query.ayat) {
            ...
            rejects(errorResponse(400, 'Silahkan isi query surat dan ayat, contoh: ?surat=1&ayat=3'))
        }
    }),
}
